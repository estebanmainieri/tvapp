...
function SvgIcon({ size = 20, color = '#fff', path, viewBox = '0 0 24 24' }: IconProps & { path: string; viewBox?: string }) {
  if (Platform.OS === 'web') {
    return (
      <div
  ...
  // Native fallback — simple colored box
...
export function GlobeIcon({ size = 20, color = '#fff', label }: IconProps & { label?: string }) {
  if (Platform.OS === 'web') {
